...
export const DEFAULT_EDITS = {
  ...
  textBold: true,
  textShadow: true,
  // Logo / image watermark (the image itself is passed to render/toBlob separately)
  logoSize: 22, // % of output width
  logoOpacity: 90, // %
  logoPos: "br",
};
...
// `logo` is an optional HTMLImageElement drawn as a watermark.
export function render(img, edits, canvas, logo = null) {
  ...
  if (logo) drawLogo(ctx, w, h, e, logo);
  drawText(ctx, w, h, e);
  return { w, h };
}

// Draw a logo/image watermark at the chosen anchor.
function drawLogo(ctx, w, h, e, logo) {
  const lw = Math.max(8, (e.logoSize / 100) * w);
  const ar = logo.naturalHeight / logo.naturalWidth || 1;
  const lh = lw * ar;
  const pad = w * 0.03;
  const pos = e.logoPos || "br";
  let x, y;
  if (pos[1] === "l") x = pad; else if (pos[1] === "c") x = (w - lw) / 2; else x = w - lw - pad;
  if (pos[0] === "t") y = pad; else if (pos[0] === "m") y = (h - lh) / 2; else y = h - lh - pad;
  ctx.save();
  ctx.globalAlpha = Math.min(1, Math.max(0, e.logoOpacity / 100));
...
export function toBlob(img, edits, canvas, logo = null) {
  const e = { ...DEFAULT_EDITS, ...edits };
  render(img, e, canvas, logo);
  const mime =
