  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-ink flex items-center gap-2"><History className="h-6 w-6 text-brand-500" /> Histórico</h1>
          <p className="mt-1 text-sm text-subtle">Seus últimos arquivos e favoritos, salvos localmente no navegador.</p>
        </div>
      ...
      <div className="flex gap-2">
        <button onClick={() => setTab("all")} className={`chip ${tab === "all" ? "!border-brand-200 !bg-brand-50 !text-brand-500" : ""}`}>Recentes ({items.length})</button>
        <button onClick={() => setTab("fav")} className={`chip ${tab === "fav" ? "!border-brand-200 !bg-brand-50 !text-brand-500" : ""}`}><Star className="h-3.5 w-3.5" /> Favoritos ({favs.length})</button>
      </div>
            ...
            {list.map((it) => {
              ...
              return (
                <motion.div key={it.id} layout initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="card p-4 flex items-center gap-3">
                  <div className="h-12 w-12 shrink-0 grid place-items-center rounded-xl overflow-hidden bg-brand-50 text-brand-500">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-subtle">{formatBytes(it.size)} · {new Date(it.at).toLocaleDateString("pt-BR")}</div>
                  </div>
