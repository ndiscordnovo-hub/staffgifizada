...
function Toggle({ label, desc, value, onChange }) {
  return (
    <div className="flex items-center justify-between gap-4 py-3 border-b border-white/5 last:border-0">
      ...
      <button onClick={() => onChange(!value)} className={`relative h-6 w-11 shrink-0 rounded-full transition-colors ${value ? "bg-brand-500" : "bg-white/15"}`}>
...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        <h1 className="text-2xl font-bold text-white flex items-center gap-2"><Settings className="h-6 w-6 text-brand-300" /> Configurações</h1>
        <p className="mt-1 text-sm text-white/45">Preferências padrão para novas edições.</p>
      </div>
      ...
      <Panel title="Exportação padrão" icon={Save}>
        ...
        <div className="field-label mt-4">Qualidade padrão: <span className="text-white/70">{Math.round(s.defaultQuality * 100)}%</span></div>
      ...
      <div className="card p-4 flex gap-3">
        <Info className="h-5 w-5 text-brand-300 shrink-0 mt-0.5" />
